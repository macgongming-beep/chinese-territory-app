...
> 마지막 업데이트: 2026-08-30 — 긴급 쓰기 재개 상태, 후속 보안 SQL 준비, 이미지 정리 함수 배포

---

## 🆕 최근 변경 사항 (2026-08-29)

### 보안 — 차단 후 긴급 쓰기 재개 상태 ⚠ (2026-08-30)

차단은 적용됐지만, 만료 토큰 사용자가 봉사 중 저장하지 못해 긴급 정책으로 쓰기를
다시 열었다. **현재는 닫힌 상태가 아니다.** `app_users`만 긴급 개방에서 제외했다.

```
EMERGENCY_open_*       26개 (TEMP 정책과 OR로 합쳐져 쓰기 열림)
...
9개를 닫아 **56개**로 줄였고, 이후 방문자 변경 함수 추가로 현재 실측은 57개다. 나머지 대부분은 트리거 함수(직접 호출 불가)이거나
권한 검사가 있다.
...
⚠ 임시로 **회수만 해 둔 다섯**이 있다 (관리자 화면이 부르던 것). 권한을 넣은
후속 SQL은 준비했지만 운영에는 아직 적용하지 않았다:
...
→ **설정 → 데이터 관리의 '실행' 과 알림 설정의 전역 항목이 지금 오류를 낸다.**
   `20260830_1700`, `1710`, `1720` 마이그레이션과 검증 SQL을 테스트 DB에서 먼저 확인한다.
   `_TEST_20260830_관리함수_권한_매트릭스.sql`은 성공한 파괴적 호출도
   서브트랜잭션으로 즉시 롤백한다.
   테스트 DB 권한 매트릭스·API smoke 통과. 이 과정에서 방문기록 날짜 컬럼을 text와
   비교해 미리보기·삭제·자동 초기화가 실패하던 기존 버그도 date 비교로 수정했다.
