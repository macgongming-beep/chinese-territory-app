...
| 채팅 | ✓ | ✓ | ✓ | ✓ |
| 알림 | ✓ | ✓ | ✓ | ✓ |
| 배정 — 구역 카드 | — | ✓ (일정→봉사자) | ✓ (인도자→카드) | ✓ |
| 배정 — 비공식 (이미지) 🛠 | — | ✓ (일정→봉사자) | ✓ (자료 풀 관리) | ✓ |
| 배정 — 식당 봉사 🛠 | — | ✓ (일정→봉사자) | — | ✓ |
| 구역 관리 | — | ✓ (담당만) | ✓ 전체 | ✓ |
| 지도 | ✓ (본인 배정 카드만) | ✓ | ✓ | ✓ |
| 공지 | ✓ (읽기) | ✓ (쓰기) | ✓ | ✓ |
...
배정 흐름은 **2단계 위계** 구조 (관리자 → 인도자 → 봉사자).

### 4.1 관리자 → 인도자에게 구역 카드 배정
- **트리거**: 배정 탭 (admin) → `MobileAdminAssignment`
- **시퀀스**:
  1. 인도자 선택 (1단계 탭)
  2. 전체 구역 목록 (2단계 탭) → 카드 선택/해제
  3. `setMultipleCardLeaders()` → `card_leader_assignments` 동기화
- **결과**: 카드 `assignedLeader`/`assignedLeaders` 갱신
- **효과**: 해당 인도자의 "구역" 탭에 담당 카드로 표시됨

### 4.2 인도자 → 봉사자에게 배정 (일정 단위) — [신 모델, 확정안]

**탭 3개 자유 순서** (현재 1→2→3 강제 흐름에서 변경 예정):

```
[ 구역 ][ 팀 구성 ][ 참가자 ]
```

#### 4.2.1 팀 구성 탭 (메인 워크스페이스)
- 팀 카드 N개 만들기 / 편집 / 삭제
- 각 팀 카드 안 섹션:
  - **구역** (구역 카드 / 비공식 / 식당 혼합 표시, 종류별 색 점)
    - `[+ 구역 추가]` → 구역 탭으로 이동, 컨텍스트 배너 "팀 N에 추가 중"
  - **팀원**
    - `[+ 팀원 추가]` → 참가자 탭으로 이동, 동일 컨텍스트
...
- **구역 (카드)**: 검색/지역 필터 → 다중 선택
- **비공식**: 이미지 자료 풀에서 선택 (관리자 업로드 + 인도자 업로드 가능)
- **식당**: 전체 상가(buildings.type='상가')에서 검색 → 단일 식당 선택 + 인원수

**진입 방식 2가지**:
- A) 탭 클릭으로 직접 진입 → 선택 시 **"미배정 풀"** 에 쌓임
- B) 팀에서 `[+ 구역 추가]` 클릭 → 컨텍스트 배너 표시 → 선택 시 해당 팀에 즉시 추가

#### 4.2.3 참가자 탭
- 일정 신청자 + 인도자가 수동 추가한 사람 목록
- 각자 어느 팀인지 표시, 팀 변경 가능

#### 4.2.4 자동 알림
- `notify_on_card_assignment()` 트리거 → 배정 받은 봉사자에게 인앱+푸시
- `type='assignment'`, link=`/territory?assignmentEvent={id}`
- 비공식/식당도 같은 트리거 패턴 (4.3, 4.4 참조)

### 4.3 [구현 예정] 비공식 증거 (이미지) 배정
- **결정 사항**:
  - 자료 형식: **이미지 파일** (PDF/QR/전단지 스캔 등)
  - 자료 풀: 관리자가 업로드 + 인도자도 업로드 가능
  - 향후 확장: "비공식 증거 카드" (구역 카드 비슷한 영역 단위) 별도 추가 가능
- **데이터 모델**:
```sql
informal_assets (
  id, name, image_url, image_path,
  uploaded_by, created_at, archived bool
)
...
```
- **UI 위치**: 구역 탭 → 비공식 서브탭 (4.2.2)
- **봉사자 표시**: 활동 카드 안 "구역" 섹션에 🟪 색 태그 + 이미지 미리보기
...
### 4.5 [공통] 팀 (event_teams) 신규 도입
- 4.2의 팀 단위가 DB 에 명시되어야 4.3/4.4 도 깔끔
- **데이터 모델**:
```sql
event_teams (
...
배정 탭은 **다음 항목들로 확장 예정**. 4.2 의 신 모델 (탭 자유 순서 + 구역 탭의 서브탭 3개) 로 통일.

| 항목 | 상태 | 관련 흐름 |
|---|---|---|
| 배정 신 모델 (탭 자유 + 서브탭) | 🛠 구현 예정 | 4.2 |
...
**디자인 시점에 미리 고려할 것**:
- 활동 화면 카드 행: 종류별 색 점 (구역 🟦 / 비공식 🟪 / 식당 🟧)
- 팀 카드: 한 줄에 종류 섞여 표시되어도 깨지지 않게
- 구역 탭: 서브탭 + picker + 컨텍스트 배너 UI 패턴 선정
