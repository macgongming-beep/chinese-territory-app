...
### 4.2 인도자 → 봉사자에게 카드 배정 (일정 단위)
- **트리거**: 배정 탭 (leader) → `MobileLeaderAssignment`
- **시퀀스**:
  1. 오늘/예정 봉사 일정 선택
  2. 1단계: 사용할 카드 선택 (4.1에서 본인이 받은 카드 중)
  3. 2단계: 팀 구성 (드래그/배치)
  4. 3단계: 참가자 추가
  5. "배정 공유" 또는 "배정 확정"
...
### 4.3 [향후] 비공식 증거 (이미지 파일) 배정
- **상태**: 📋 계획 중 (미구현)
- **목적**: 봉사 모임에서 인도자가 봉사자에게 비공식 증거 자료(QR 카드, 전단지 이미지, 안내문 등)를 일정 단위로 배포
- **예상 구조**:
  - 새 테이블: `informal_witness_assets` (id, name, image_url, uploader, ...)
  - 새 테이블: `event_informal_witness_assignments` (event_id, user_name, asset_id)
  - 배정 흐름 4.2 의 3단계 직후 또는 별도 탭으로 자산 첨부
- **봉사자 측 표시**: 활동 카드 안에 "오늘 받은 자료" 섹션 추가
- **푸시 알림**: 배정 트리거에서 같이 처리 가능

### 4.4 [향후] 식당 봉사 (Restaurant Service) 배정
- **상태**: 📋 계획 중 (미구현)
- **목적**: 봉사 모임에서 점심 식당, 음료 등 식사 관련 봉사를 별도 리스트로 관리
- **예상 구조**:
  - 새 테이블: `restaurants` (id, name, address, phone, lat, lng, memo)
  - 새 테이블: `event_restaurant_assignments` (event_id, restaurant_id, time_slot, party_size)
  - 일정 상세에 "식당" 섹션 노출
- **봉사자 측 표시**: 활동/일정 카드에 "오늘의 식당" 표시 + 길찾기
- **인도자 작업**: 식당 선택 + 인원수 + 시간 설정
...
배정 탭은 **다음 항목들로 확장 예정**. 현재는 구역 카드만 다루지만, 모두 같은 "일정(event) 기반 배정" 패턴으로 추가될 예정.

| 항목 | 상태 | 관련 흐름 |
|---|---|---|
| 구역 카드 배정 | ✅ 운영 중 | 4.1, 4.2 |
| 비공식 증거 이미지 배정 | 📋 계획 | 4.3 |
| 식당 봉사 배정 | 📋 계획 | 4.4 |

**공통 패턴 (재사용 권장)**:
- 일정(`calendar_events`) 선택 → 참가자 추출 → 자산/대상 매핑 → INSERT 트리거 → 푸시
- UI: `MobileLeaderAssignment` 의 3단계 흐름 확장 또는 탭 전환식
- 알림 type: `assignment` 재사용 가능 (또는 `assignment_asset`, `assignment_restaurant` 세분화)

**디자인 시점에 미리 고려할 것**:
- 활동 화면 카드 행에 "구역 / 자료 / 식당" 3섹션 들어갈 공간 확보
- 배정 탭 진입 시 종류 선택 (탭 또는 세그먼트) UI 패턴 선정
