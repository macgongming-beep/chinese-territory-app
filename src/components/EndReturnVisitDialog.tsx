  ...
  return (
    <div className="rv-end-backdrop" onClick={onClose}>
      <section className="rv-end-sheet" role="dialog" aria-modal="true" aria-labelledby="rv-end-title" onClick={(event) => event.stopPropagation()}>
        <header>
          <div>
            <h2 id="rv-end-title">{msg('정기방문 종료')}</h2>
            <p>{visit.nickname || visit.displayName}</p>
          </div>
          <button type="button" onClick={onClose} aria-label={msg('닫기')}>×</button>
        </header>

        <div className="rv-end-section">
          <strong>{msg('왜 종료하나요?')}</strong>
        ...
        <div className="rv-end-section">
          <label className="rv-end-label" htmlFor="rv-place-issue">{msg('장소 정보에도 문제가 있나요?')} <small>{msg('선택')}</small></label>
          <select id="rv-place-issue" value={issueType} onChange={(event) => setIssueType(event.target.value as PlaceIssueType | '')}>
            <option value="">{msg('문제 없음')}</option>
            {issues.map((item) => <option key={item.value} value={item.value}>{msg(item.label)}</option>)}
          </select>
          {issueType && (
            <textarea
              value={note}
              onChange={(event) => setNote(event.target.value)}
              placeholder={issueType === 'details_wrong' ? msg('올바른 주소·이름·호수를 적어 주세요.') : msg('관리자가 확인할 내용을 적어 주세요.')}
              rows={3}
            />
          )}
        </div>

        <p className="rv-end-preserve">{msg('건물·세대와 기존 방문기록은 그대로 남습니다.')}</p>
        <div className="rv-end-actions">
          <button type="button" onClick={onClose}>{msg('취소')}</button>
          <button
            type="button"
            className="primary"
            disabled={saving}
          ...
          >{saving ? msg('처리 중…') : issueType ? msg('종료하고 요청 보내기') : msg('정기방문 종료')}</button>
        </div>
      </section>
    </div>
  )
