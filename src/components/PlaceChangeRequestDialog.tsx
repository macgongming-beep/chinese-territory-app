  ...
  return (
    <div className="rv-end-backdrop" onClick={onClose}>
      <section className="rv-end-sheet" role="dialog" aria-modal="true" aria-labelledby="place-request-title" onClick={(event) => event.stopPropagation()}>
        <header>
          <div>
            <h2 id="place-request-title">{msg('자료 수정 요청')}</h2>
        ...
        <div className="rv-end-section">
          <label className="rv-end-label" htmlFor="place-request-type">{msg('어떤 문제가 있나요?')}</label>
          <select id="place-request-type" value={requestType} onChange={(event) => setRequestType(event.target.value as PlaceIssueType)}>
            {issues.map((issue) => <option key={issue.value} value={issue.value}>{msg(issue.label)}</option>)}
          </select>
          <textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder={msg('관리자가 확인할 내용을 적어 주세요.')} rows={3} />
        </div>
        <p className="rv-end-preserve">{msg('요청만 보내며 건물·세대와 방문 기록은 지금 삭제되지 않습니다.')}</p>
        <div className="rv-end-actions">
          ...
          >{saving ? msg('보내는 중…') : msg('요청 보내기')}</button>
        </div>
      </section>
    </div>
  )
