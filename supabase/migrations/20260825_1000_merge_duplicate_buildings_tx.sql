...
begin
  ...
  for v_group in
    ...
    select
      p.primary_id,
      p.absorbed_ids,
      -- 기준 건물과 흡수될 건물에서 정규화한 호수를 모아 겹침을 찾는다
      (
        select coalesce(array_agg(distinct u.number order by u.number), '{}')
        from public.units u
        where u.building_id = any(p.absorbed_ids)
          and public.normalize_unit_number(u.number) in (
            select public.normalize_unit_number(pu.number)
            from public.units pu where pu.building_id = p.primary_id
          )
      ) as conflicting
    from picked p
  loop
