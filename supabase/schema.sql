-- ⚠⚠ 낡았다. 쓰지 말 것. ⚠⚠
--
-- 2026-08-24 확인: 이 파일에는 테이블이 12개뿐이다. 실제 운영은 39개다.
-- 초기 스키마의 기록으로만 남긴다.
--
-- **빈 DB 에 앱을 세우려면 supabase/baseline.sql 을 쓴다** (+ baseline-extras.sql).
-- 절차는 docs/새-회중-설치.md 에 있다.
...
create table public.units (
  ...
  status      text not null default '미방문' check (status in ('미방문', '만남', '부재', '대상외', '거절', '확인필요')),
...
create table public.visit_histories (
  ...
  visitor_name text not null,
  result       text not null check (result in ('만남', '부재', '대상외', '거절', '확인필요')),
