-- CSV 건물 올리기를 한 트랜잭션으로. **한 트랜잭션으로 돈다.**
-- 테스트 DB 에서 롤백을 실제로 확인했다 (세대 삽입 직전에 터뜨림 → 건물도 안 남음).
...
-- ═══ commit 뒤에 따로 돌릴 확인 (전부 true) ═══
-- select
--   (select count(*) from pg_proc where proname='import_building_tx') = 1 as RPC_있음,
--   (select position('관리자만' in prosrc) > 0 from pg_proc where proname='import_building_tx') as 관리자검사,
--   (select position('저녁' in prosrc) > 0 from pg_proc where proname='import_building_tx') as time_slot_기본값,
--   (select not has_function_privilege('anon','public.import_building_tx(uuid,jsonb,jsonb)','execute')) = false as anon_실행가능;
